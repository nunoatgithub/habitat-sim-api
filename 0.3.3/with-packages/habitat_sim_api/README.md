...
for name in dir(habitat_sim_api):
    obj = getattr(habitat_sim_api, name)
...
This package mirrors habitat-sim version **0.3.3**. The API surface should remain compatible across minor versions, but check the habitat-sim changelog for breaking changes.
